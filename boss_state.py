...
from map      import DungeonBossMap
...
def enter():
    ...
    server.map = DungeonBossMap()
    game_world.add_object(server.map, 0)
    
    ground = Ground(0, 0, 1600, 140)
    server.ground.append(ground)
    game_world.add_objects(server.ground, 0)
    
    stepstone = Stepstone(485, 290, 625)
    server.stepstone.append(stepstone)
    stepstone = Stepstone(640, 685, 625//2)
    server.stepstone.append(stepstone)
    stepstone = Stepstone(235, 585, 200)
    server.stepstone.append(stepstone)
    stepstone = Stepstone(235, 420, 200)
    server.stepstone.append(stepstone)
    stepstone = Stepstone(1155, 585, 200)
    server.stepstone.append(stepstone)
    stepstone = Stepstone(1155, 420, 200)
    server.stepstone.append(stepstone)
    
    game_world.add_objects(server.stepstone, 0)
